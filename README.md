...
- **OldLondon**: uniquement le gros titre "Kom Bouffe" en haut de page. C'est la police K-Fêt (envoyée par le président en .ttf/.otf). Elle est intégrée directement dans index.html via une règle @font-face encodée en base64 (comme le logo), donc pas de fichier de police séparé à uploader sur GitHub: tout reste dans ce seul fichier. Le zip original contenait aussi une variante OldLondonAlternate (glyphes différents) et deux versions .otf ; c'est OldLondon.ttf qui a été retenu.
- **Ocean6 (Ocean 6 Brushed Black Edition)**: les petits titres de section et les noms de recettes partout où ils apparaissent (liste par mois, détail d'une recette, résultats de recherche par ingrédient), toujours en majuscules (text-transform:uppercase). Même principe que OldLondon : police envoyée en .ttf, intégrée en base64 via une seconde règle @font-face, pas de fichier séparé.
