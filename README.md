...
- **OldLondon**: le titre "Kom Bouffe", les petits titres de section et les noms de recettes partout où ils apparaissent (liste par mois, détail d'une recette, résultats de recherche par ingrédient). C'est la police K-Fêt (fichier `.ttf`/`.otf`), elle est intégrée directement dans `index.html` via une règle `@font-face` encodée en base64 (comme le logo), donc pas de fichier de police séparé à uploader sur GitHub — tout reste dans ce seul fichier. Le zip original contenait aussi une variante `OldLondonAlternate` (glyphes différents) et deux versions `.otf` ; c'est `OldLondon.ttf` qui a été retenu. Pour changer de variante, il faut réencoder le nouveau fichier en base64 et remplacer le contenu de la règle `@font-face`.
